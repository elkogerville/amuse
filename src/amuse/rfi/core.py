...
class CodeFunction(object):
    ...
    def _async_request(self, *arguments_list, **keyword_arguments):
        ...
        def handle_result(function):
            try:
                ...
                raise exceptions.CodeException(
                    "Exception when calling legacy code '{0}', exception was '{1}'".format(
                        self.specification.name, ex
                    )
                )
            result = self.converted_results(dtype_to_result, handle_as_array)
            return result
    ...
    def converted_results(self, dtype_to_result, must_handle_as_array):
        ...
        if number_of_outputs == 1 and result_type is None:
    ...
    def converted_keyword_and_list_arguments(self, arguments_list, keyword_arguments):
        ...
        input_parameters_seen = set(
            [x.name for x in self.specification.input_parameters]
        )
...
def get_function_specification(
    ...
    for arg, value in in_arg.items():
        ...
        function.addParameter(
            arg, dtype=dtype, direction=function.IN, unit=unit, default=default
        )
    for arg, value in inout_arg.items():
        dtype, unit, default = derive_dtype_unit_and_default(value)
        function.addParameter(
            arg, dtype=dtype, direction=function.INOUT, unit=unit, default=default
        )
    for arg, value in out_arg.items():
        dtype, unit, default = derive_dtype_unit_and_default(value)
        function.addParameter(
            arg, dtype=dtype, direction=function.OUT, unit=unit, default=default
        )
...
def remote_function(f=None, must_handle_array=False, can_handle_array=False):
    ...
    return legacy_function(
        simplified_function_specification(
            must_handle_array=must_handle_array, can_handle_array=can_handle_array
        )(f)
    )
...
class LegacyFunctionSpecification(object):
    ...
    def __init__(
        self, counter=[0]
    ):  # counter serves to be able to put specs in anything resembling sane order (=input order)
        counter[0] += 1
    ...
    def addParameter(
        self, name, dtype="i", direction=IN, description="", default=None, unit=None
    ):
        """
        ...
        parameter = ParameterSpecification(
            name, dtype, direction, description, default, unit
        )
    ...
    def add_input_parameter(self, parameter):
        has_default_parameters = any(
            [x.has_default_value() for x in self.input_parameters]
        )
        if has_default_parameters and not parameter.has_default_value():
            raise exceptions.AmuseException(
                "non default argument '{0}' follows default argument".format(
                    parameter.name
                )
            )
...
class CodeInterface(OptionalAttributes):
    ...
    def _start(
        self, name_of_the_worker="worker_code", interpreter_executable=None, **options
    ):
        ...
        self.channel.polling_interval_in_milliseconds = (
            self.polling_interval_in_milliseconds
        )
        ...
        if self.working_directory:
            ...
            if result != 0:
                raise Exception(
                    f"Changing to working directory {self.working_directory} failed"
                )
        ...
        if self.channel.is_polling_supported():
            if self.polling_interval_in_milliseconds > 0:
                self.internal__set_message_polling_interval(
                    int(self.polling_interval_in_milliseconds * 1000)
                )
    ...
    @option(
        choices=["mpi", "remote", "distributed", "sockets", "local"],
        sections=("channel",),
    )
    ...
    def channel_factory(self):
        if self.channel_type == "mpi":
            if MpiChannel.is_supported():
                ...
                warnings.warn(
                    "MPI (unexpectedly?) not available, falling back to sockets channel"
                )
            ...
            raise exceptions.AmuseException(
                "Cannot create a channel with type {0!r}, type is not supported".format(
                    self.channel_type
                )
            )
    ...
    def set_working_directory():
        function = LegacyFunctionSpecification()
        function.addParameter(
            "working_directory", dtype="string", direction=function.IN
        )
        function.result_type = "int32"
        function.internal_provided = True
        return function
    ...
    def get_working_directory():
        function = LegacyFunctionSpecification()
        function.addParameter(
            "working_directory", dtype="string", direction=function.OUT
        )
        function.result_type = "int32"
        function.internal_provided = True
        return function
...
class CodeWithDataDirectories(object):
    ...
    def __init__(self):
        if self.channel_type == "distributed":
            warnings.warn(
                "Code with DistributedChannel wants to make output directory, check.."
            )
        else:
    ...
    def get_data_directory(self):
        ...
        if self.input_data_root_directory:
            return os.path.join(
                self.input_data_root_directory, self.module_name, "input"
            )
        else:
    ...
    def get_output_directory(self):
        ...
        if self.output_data_root_directory:
            return os.path.join(
                self.output_data_root_directory, self.module_name, "output"
            )
        else:
            working_directory = self.get_working_directory()["working_directory"]
            return os.path.join(
                working_directory, "__amuse_code_output", self.module_name
            )  # note problem for multiple codes
...
class PythonCodeInterface(CodeInterface):
    ...
    def __init__(self, implementation_factory=None, name_of_the_worker=None, **options):
        if self.channel_type == "distributed":
            print(
                "Warning! Distributed channel not fully supported by PythonCodeInterface yet"
            )
    ...
    def _start(self, name_of_the_worker="worker_code", **options):
        ...
        if name_of_the_worker is None:
            if self.implementation_factory is None:
                raise exceptions.CodeException(
                    "Must provide the name of a worker script or the implementation_factory class"
                )
            name_of_the_worker = self.make_executable_script_for(
                self.implementation_factory
            )
            if not options.setdefault("dynamic_python_code", True):
                raise exceptions.CodeException(
                    "dynamic code set to false, but python code generated"
                )

        if self.use_python_interpreter:
            CodeInterface._start(
                self,
                name_of_the_worker=name_of_the_worker,
                interpreter_executable=self.python_interpreter,
                **options,
            )
        else:
    ...
    @classmethod
    def new_executable_script_string_for(
        cls, implementation_factory, channel_type="mpi"
    ):
...
class CodeFunctionWithUnits(CodeFunction):
    ...
    def __call__(self, *arguments_list, **keyword_arguments):
        ...
        try:
            ...
            dtype_to_result, output_encoded_units = self.interface.channel.recv_message(
                call_id, self.specification.id, handle_as_array, has_units=True
            )
        except Exception as ex:
            CODE_LOG.info(
                "Exception when calling function '{0}', of code '{1}', exception was '{2}'".format(
    ...
    def _async_request(self, *arguments_list, **keyword_arguments):
        ...
        def handle_result(function):
            ...
            output_units = self.convert_floats_to_units(output_encoded_units)
            return self.converted_results(
                dtype_to_result, handle_as_array, output_units
            )
