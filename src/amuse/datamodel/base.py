...
class RandomNumberUniqueKeyGenerator(KeyGenerator):
    ...
    def __init__(self, number_of_bits=None, random=None):
        ...
        if number_of_bits > 64:
            raise exceptions.AmuseException(
                "number of bits is larger than 64, this is currently unsupported!"
            )
    ...
    def __next__(self):
        return numpy.array(
            [random.getrandbits(self.number_of_bits)], dtype=numpy.uint64
        )[0]
    ...
    def next_set_of_keys(self, length):
        ...
        try:
            ...
            return numpy.array(
                [random.getrandbits(self.number_of_bits) for i in range(length)],
                dtype="uint64",
            )
...
def set_random_key_generator(
    number_of_bits=RandomNumberUniqueKeyGenerator.DEFAULT_NUMBER_OF_BITS,
):
    global UniqueKeyGenerator
...
class VectorAttributeValue(AbstractAttributeValue):
    ...
    def _set_values_for_entities(self, value):
        ...
        split = [
            x.reshape(x.shape[0] if len(x.shape) <= 2 else x.shape[:-1]) for x in split
        ]
        ...
        self.collection.set_values_in_store(
            self.indices, self.attribute_names, list_of_values
        )
...
class VectorAttribute(DerivedAttribute):
    ...
    def set_values_for_entities(self, instance, value):
        ...
        instance.set_values_in_store(
            instance.get_all_indices_in_store(), self.attribute_names, list_of_values
        )
...
class CalculatedAttribute(DerivedAttribute):
    ...
    def get_values_for_entities(self, instance):
        values = instance.get_values_in_store(
            instance.get_all_indices_in_store(), self.attribute_names
        )
    ...
    class BoundParticleFunctionAttribute(object):
        ...
        def __call__(self, *list_arguments, **keyword_arguments):
            return self._function(
                self.particles, self.particle, *list_arguments, **keyword_arguments
            )
        ...
        def __call__(self, *list_arguments, **keyword_arguments):
            ...
            if self._name in cached_results.results:
                ...
                result = self._function(
                    self.particles, *list_arguments, **keyword_arguments
                )
...
class FunctionAttribute(DerivedAttribute):
    def __init__(self, particles_function=None, particle_function=None):
        self.particles_function_attribute = new_particles_function_attribute_with_doc(
            particles_function
        )
        self.particle_function_attribute = new_particle_function_attribute_with_doc(
            particle_function
        )
    ...
    def __getstate__(self):
        return (
            self.particles_function_attribute._function,
            self.particle_function_attribute._function,
        )
...
class CachingFunctionAttribute(DerivedAttribute):
    def __init__(
        self, name_of_the_attribute, particles_function=None, particle_function=None
    ):
        self.particles_function_attribute = (
            new_caching_particles_function_attribute_with_doc(
                name_of_the_attribute, particles_function
            )
        )
        self.particle_function_attribute = new_particle_function_attribute_with_doc(
            particle_function
    ...
    def __setstate__(self, state):
        if state[0]:
            self.particles_function_attribute = (
                new_caching_particles_function_attribute_with_doc(
                    state[0].__name__, state[0]
                )
            )
        if state[1]:
            self.particle_function_attribute = new_particle_function_attribute_with_doc(
                state[1].__name__, state[1]
            )
    ...
    def __getattr__(self, name_of_the_attribute):
        ...
        try:
            return self._store._convert_to_entities_or_quantities(
                self._store.get_all_values_of_attribute_in_store_async(
...
class AbstractSet(object):
    ...
    def __init__(self, original=None):
        ...
        object.__setattr__(
            self, "_derived_attributes", CompositeDictionary(derived_attributes)
        )
    ...
    def __setattr__(self, name_of_the_attribute, value):
        ...
        if name_of_the_attribute in self._derived_attributes:
            self._derived_attributes[name_of_the_attribute].set_values_for_entities(
                self, value
            )
        else:
            self.set_values_in_store(
                self.get_all_indices_in_store(),
                [name_of_the_attribute],
                [self._convert_from_entities_or_quantities(value)],
            )
    ...
    def get_all_values_of_attribute_in_store_async(self, attribute):
        return self.get_values_in_store_async(
            self.get_all_indices_in_store(), [attribute]
        )[0]
    ...
    def add_vector_attribute(self, name_of_the_attribute, name_of_the_components):
        self._derived_attributes[name_of_the_attribute] = VectorAttribute(
            name_of_the_components
        )
    ...
    def add_global_vector_attribute(cls, name_of_the_attribute, name_of_the_components):
        ...
        """
        cls.GLOBAL_DERIVED_ATTRIBUTES[name_of_the_attribute] = VectorAttribute(
            name_of_the_components
        )

    def add_calculated_attribute(
        self, name_of_the_attribute, function, attributes_names=None
    ):
        """
        ...
        self._derived_attributes[name_of_the_attribute] = CalculatedAttribute(
            function, attributes_names
        )

    @classmethod
    def add_global_calculated_attribute(
        cls, name_of_the_attribute, function, attributes_names=None
    ):
        """
        Define a *global* vector attribute, coupling two or more scalar attributes into
        one vector attribute. The vector will be defined for all particle sets
        ...
        """
        cls.GLOBAL_DERIVED_ATTRIBUTES[name_of_the_attribute] = CalculatedAttribute(
            function, attributes_names
        )

    def add_function_attribute(
        self, name_of_the_attribute, function, function_for_particle=None
    ):
        """
        ...
        self._derived_attributes[name_of_the_attribute] = FunctionAttribute(
            function, function_for_particle
        )

    def add_caching_function_attribute(
        self, name_of_the_attribute, function, function_for_particle=None
    ):
        """
        ...
        self._derived_attributes[name_of_the_attribute] = CachingFunctionAttribute(
            name_of_the_attribute, function, function_for_particle
        )

    @classmethod
    def add_global_function_attribute(
        cls, name_of_the_attribute, function, function_for_particle=None
    ):
        """
        Define a function attribute, adding a function to the particles
        ...
        cls.GLOBAL_DERIVED_ATTRIBUTES[name_of_the_attribute] = FunctionAttribute(
            function, function_for_particle
        )

    @classmethod
    def add_global_caching_function_attribute(
        cls, name_of_the_attribute, function, function_for_particle=None
    ):
        """
        Define a function attribute, adding a function to the particles
        ...
        cls.GLOBAL_DERIVED_ATTRIBUTES[name_of_the_attribute] = CachingFunctionAttribute(
            name_of_the_attribute, function, function_for_particle
        )
    ...
    def copy(
        ...
        filter_attributes=lambda particles, x: True,
    ):
    ...
    def copy_to_memory(self):
        warnings.warn(
            "deprecated, pleasy change 'copy_to_memory' to 'copy'", DeprecationWarning
        )
        return self.copy()
    ...
    def __sub__(self, particles):
        ...
        for key in subtract_keys:
            if key in new_keys:
                ...
                raise exceptions.AmuseException(
                    "Unable to subtract a particle, because "
    ...
    def synchronize_to(self, other_particles):
        ...
        if added_keys:
            ...
            attributes = [
                x for x in attributes if x not in other_particles._derived_attributes
            ]
...
class LinkedArray(numpy.ndarray):
        ...
        for x in result:
            if x is None:
                ...
                if id(container) in memento:
                    ...
                    copy_of_container = container.copy(
                        memento, keep_structure, filter_attributes
                    )
    ...
    def as_set(self):
        ...
        masked_keys = numpy.ma.masked_array(numpy.asarray(keys, dtype=dtype), mask=mask)
...
class FixedLinkedArray(LinkedArray):
    ...
    def copy(
        ...
        if id(self.linked_set) in memento:
            ...
            copy_of_container = self.linked_set.copy(
                memento, keep_structure, filter_attributes
            )
    ...
    def copy_with_link_transfer(
        ...
        if must_copy:
            ...
            new_container = from_container.copy(
                memento, keep_structure=True, filter_attributes=filter_attributes
            )
        else:
    ...
    def set_particles(self, index, value):
        if not self.linked_set.are_all_keys_in_set(value.as_set().key):
            raise Exception(
                "trying to link to a particle that is not in the linked set"
            )
