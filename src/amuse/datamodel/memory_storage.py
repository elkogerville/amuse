...
class InMemoryAttributeStorage(AttributeStorage):
    ...
    def append_to_storage(self, keys, attributes, values):
        for attribute, values_to_set in zip(attributes, values):
            ...
            try:
                storage.set_values(
                    slice(len(self.particle_keys), len(self.particle_keys) + len(keys)),
                    values_to_set,
                )
            except Exception as ex:
                raise AttributeError(
                    "exception in setting attribute '{0}', error was '{1}'".format(
                        attribute, ex
                    )
                )
        ...
        self.particle_keys = numpy.concatenate(
            (self.particle_keys, numpy.array(list(keys), dtype="uint64"))
        )
        self.reindex()
    ...
    def copy(self):
        ...
        for (
            attribute,
            attribute_values,
        ) in self.mapping_from_attribute_to_quantities.items():
            copy.mapping_from_attribute_to_quantities[
                attribute
...
class InMemoryGridAttributeStorage(object):
    ...
    def remove_particles_from_store(self, keys):
        raise exceptions.AmuseException(
            "removing points from the grid is not implemented"
        )
    ...
    def set_values_in_store(self, indices, attributes, list_of_values_to_set):
        for attribute, values_to_set in zip(attributes, list_of_values_to_set):
            if attribute in self.mapping_from_attribute_to_quantities:
                ...
                storage = InMemoryAttribute.new_attribute(
                    attribute, self.storage_shape(), values_to_set
                )
                self.mapping_from_attribute_to_quantities[attribute] = storage
                # storage.set_values(None, quantity)
    ...
    def set_values(self, indices, values):
        try:
            ...
            if not is_quantity(values):
                raise ValueError(
                    "Tried to set a non quantity value for an attribute ({0}) with a unit".format(
                        self.name
                    )
                )
            else:
...
class InMemoryUnitlessAttribute(InMemoryAttribute):
    def __init__(self, name, shape, dtype="float64"):
        ...
        self.values = numpy.zeros(shape, dtype=dtype)
...
class InMemoryStringAttribute(InMemoryUnitlessAttribute):
    def set_values(self, indices, values):
        ...
        if isinstance(values, str):
            ...
            values_as_array = numpy.asarray(
                values, dtype=numpy.dtype(self.values.dtype.kind)
            )
...
class InMemoryLinkedAttribute(InMemoryAttribute):
    def __init__(self, name, shape):
        InMemoryAttribute.__init__(self, name)
        self.values = LinkedArray(numpy.empty(shape, dtype=object))
    ...
    def increase_to_length(self, newlength):
        ...
        self.values = LinkedArray(
            numpy.concatenate([self.values, zeros_for_concatenation])
        )
