...
class UclchemInterface(CodeInterface, CommonCodeInterface, LiteratureReferencesMixIn):
    ...
    def __init__(self, mode="cpu", **options):
        CodeInterface.__init__(self, name_of_the_worker="uclchem_worker", **options)
...
class UCLchem(CommonCode):
    ...
    def _build_dict(self, tend):
        ...
        for i in range(len(self.particles.key)):
            ...
            if "temperature" in attributes:
                dictionary["initialTemp"] = self.particles.temperature.value_in(
                    units.K
                )[i]
            if "number_density" in attributes:
                dictionary["initialDens"] = self.particles.number_density.value_in(
                    units.cm**-3
                )[i]
            if "ionrate" in attributes:
                dictionary["zeta"] = self.particles.ionrate.value_in(units.cr_ion)[i]
            if "radfield" in attributes:
                dictionary["radfield"] = self.particles.radfield.value_in(units.habing)[
                    i
                ]
            dictionary["finalTime"] = tend.value_in(
                units.yr
            ) - self.uclchem_time.value_in(units.yr)
    ...
    def define_methods(self, handler):
        ...
        handler.add_method(
            "new_particle",
            (units.cm**-3, units.K, units.s**-1, units.habing),
            (
                handler.INDEX,
                handler.ERROR_CODE,
            ),
        )
        handler.add_method("delete_particle", (handler.INDEX,), (handler.ERROR_CODE,))
        handler.add_method("evolve_model", (units.yr), (handler.ERROR_CODE))
        handler.add_method("get_time", (), (units.yr, handler.ERROR_CODE))
