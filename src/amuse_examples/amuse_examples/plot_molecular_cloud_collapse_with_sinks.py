...
def make_map(sph, N=100, L=1):
    ...
    x = L * (x.flatten() - N / 2.0) / N
    y = L * (y.flatten() - N / 2.0) / N
    z = x * 0.0
    vx = 0.0 * x
    vy = 0.0 * x
    vz = 0.0 * x
...
def make_stars(cluster_particle):
    ...
    if N > 0:
        masses = new_salpeter_mass_distribution(
            N, 0.3 | units.MSun, min(100 | units.MSun, cluster_particle.mass)
        )
...
def plot_hydro_and_stars(time, sph, L=10):
    ...
    plt.imshow(
        numpy.log10(1.0e-5 + rho.value_in(units.amu / units.cm**3)),
